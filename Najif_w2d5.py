...
df=pd.read_csv(r'samplesuperstore_cleaned.csv')
...
with tab3:
    ...
    outlier_n = int(np.sum(np.abs(z_scores)>2))
...
with tab4:
    ...
    if mean_margin < 10:
        st.error(
            f"🟤Low profit margin: "
            f"(mean_margin :.1f)% - investigate discounts"
        )

    elif mean_margin < 20:
        st.warning(
        f"🟡 Moderate profit margin: "
        f"{mean_margin:.1f}% - monitor discounts"
    )
